...
Your data contract is the heart of your application. It lives at [`packages/database/src/prisma/contract.prisma`](packages/database/src/prisma/contract.prisma) and describes your models:
...
```typescript
import { db } from './packages/database/src/prisma/db';
...
export default definePrismaConfig({
  orm: ormConfig({
    contract: './packages/database/src/prisma/contract.prisma',
    db: {
...
| File | Purpose |
|---|---|
| [`packages/database/src/prisma/contract.prisma`](packages/database/src/prisma/contract.prisma) | Your data contract — define your models here |
| [`prisma.config.ts`](prisma.config.ts) | CLI configuration |
| [`packages/database/src/prisma/db.ts`](packages/database/src/prisma/db.ts) | Database client — `import { db } from './packages/database/src/prisma/db'` |
| `packages/database/src/prisma/contract.json` | Compiled contract (generated) |
| `packages/database/src/prisma/contract.d.ts` | Contract types (generated) |

### Workflow

1. Edit [`packages/database/src/prisma/contract.prisma`](packages/database/src/prisma/contract.prisma) to add or change models.
...
- **Catalogs.** When the workspace's `pnpm-workspace.yaml` defines a `catalogs` entry for `prisma` or `@prisma/orm-mongo`, pnpm uses the catalog version everywhere — `init` does too. If you wanted the published `latest` instead, update or remove the catalog entry, then re-run `pnpm install`.
